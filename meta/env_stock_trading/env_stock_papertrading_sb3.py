...
class AlpacaPaperTrading_sb3:
    def __init__(
        ...
        if agent == "ppo":
            ...
            try:
                ...
                print("Successfully loaded SB3 PPO model from", cwd)
            except Exception as e:
                raise ValueError(f"Failed to load PPO model: {e}")
        else:
    ...
    def trade(self):
        try:
            ...
            print(f"[Action Predict Error] {e}")
            return
        ...
        if self.turbulence_bool == 0:
            ...
            try:
                ...
                for position in positions:
                    ...
                    t = threading.Thread(
                        target=self.submitOrder,
                        args=(qty, position.symbol, side, respSO),
                    )
                    t.start()
                    t.join(timeout=30)
                self.stocks_cd[:] = 0
            except Exception as e:
                print(f"[Turbulence Sell Error] {e}")
